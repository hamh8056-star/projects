...
function AppHeader() {
  ...
  return (
    <header className="fixed lg:sticky top-0 left-0 right-0 z-40 bg-white/80 backdrop-blur border-b border-blue-100 shadow-sm lg:relative">
      <div className="px-3 sm:px-4 lg:px-6 xl:px-8 flex items-center h-16 justify-between">
        <div className="flex items-center gap-2 sm:gap-4 justify-end ml-auto">
          <button className="relative p-2 rounded-full hover:bg-cyan-100 transition focus:outline-none focus:ring-2 focus:ring-cyan-400">
            <Bell className="w-5 h-5 sm:w-6 sm:h-6 text-cyan-700" />
          </button>
          <div className="relative" ref={menuRef}>
            <button
              className="w-8 h-8 sm:w-9 sm:h-9 rounded-full bg-cyan-600 text-white flex items-center justify-center font-bold text-sm sm:text-lg focus:outline-none focus:ring-2 focus:ring-cyan-400"
